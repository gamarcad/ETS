...
impl MockProtocol {
    ...
    pub fn show(
        ...
        // Warning: This part of the code has been modified due to a mistake in scope
        // of the enc variable redefined in a new variable. The following fix is temporary.
        // Further investigations are required in the code.
        let mut t_3 = G2Projective::generator();
        let mut pi_2: Option<(Scalar, Scalar, Scalar)> = None;
        let (enc2, alpha) = Auditing::audit_enc(&self.user_key, &self.authority_key);
    ...
    /// Extracts the public key of the user having generated the provided proof.
    ///
    /// The provided proof may contain the encryption of the user's public key having
    /// generated the proof. If the provided proof is not embedding the encrypted public key then
    /// the auditability feature has not been properly set and `None` is returned.
    pub fn audit(&self, omega : &Omega) -> Option<G1Projective> {
        match &omega.enc {
            Some(enc) => Some(Auditing::audit_dec( &enc, &self.authority_key )),
            None => None
        }
    }
}
